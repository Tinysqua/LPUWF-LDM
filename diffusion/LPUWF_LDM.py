...
def main():
    ...
    for epoch in range(Config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                early_ffa = vae.encode_stage_2_inputs(early_ffa)
                early_ffa = early_ffa * scaling_factor
            latent_shape = list(early_ffa.shape);latent_shape[0] = Config.eval_bc
            noise = torch.randn_like(early_ffa) + 0.1 * torch.randn(early_ffa.shape[0], early_ffa.shape[1], 1, 1).to(early_ffa.device)
            timesteps = torch.randint(
                0, inferer.scheduler.num_train_timesteps, (early_ffa.shape[0],), device=early_ffa.device
            ).long()
            ffa_noised = scheduler.add_noise(early_ffa, noise, timesteps)
        ...
        if (epoch + 1) % val_interval == 0 or epoch == cf['num_epochs'] - 1:
            ...
            image, early_ffa = one_to_three(image), one_to_three(early_ffa) 
            image = torch.cat([slo, image, early_ffa], dim=-1)
            image = (make_grid(image, nrow=1).unsqueeze(0)+1)/2
            log_image = {"Early": image.clamp(0, 1)}
            save_path = j(Config.project_dir, 'image_save')
            os.makedirs(save_path, exist_ok=True)
            save_image(log_image["Early"], j(save_path, f'epoch_{epoch + 1}_Early.png'))
