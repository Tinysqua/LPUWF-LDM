    ...
    def __init__(
        ...
        for l in range(len(out_channels)):
            ...
            if l == 0:
                ...
                # gated_cnn.append(nn.Conv2d(out_ch_, out_ch_, kernel_size=3, stride=1, padding=1))
            else:
                layers.append(nn.Conv2d(in_ch_, out_ch_, kernel_size=4, stride=2, padding=1))
                # gated_cnn.append(nn.Conv2d(out_ch_, out_ch_, kernel_size=3, stride=2, padding=1))
            gated_cnn.append(nn.Conv2d(out_ch_, out_ch_, kernel_size=3, stride=1, padding=1))
